...
def imageToArray(i):
    """
    Converts a Python Imaging Library array to gdalnumeric image.
    """
...
def world2Pixel(geoMatrix, x, y):
    """
    Uses a gdal geomatrix (gdal.GetGeoTransform()) to calculate the pixel
...
## Multispectral image used to create the NDVI. Must have red and infrared bands
source = './data/advanced/NDVI-update/farm.tif'
## Outut geotiff filename
target = './data/advanced/NDVI-update/ndvi.tif'

## Load the source data as a gdalnumeric array
srcArray = gdalnumeric.LoadFile(source)

## Also load as gdal image to get geotransformation (world file) info
...
field = ogr.Open('./data/advanced/NDVI-update/field.shp')
## Must define a layer to keep OGR happy
layer = field.GetLayer('field')
...
## Create a blank image of the correct size that serves as our mask
...
## Map points to pixels for drawing the field boundary on a blank 8-bit
...
## Loop through geometry and turn the points into an easy-to-manage Python list
